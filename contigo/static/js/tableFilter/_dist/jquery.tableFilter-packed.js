...
jQuery.extend(jQuery.tblFilter,{
    ...
    prototype: {
        ...
        guessDataType: function(rows,c){ // null rows suck. To avoid reparsing, test 10 rows
            ...
            if ((same/tc) < .75) {
                // Modified by VF from "string" to "number"
		return "number";
                } else {
                return dt1;
            };
            ...
            for (var c=0;c<cLen;c++){
                ...
                if (textLength) {
                    switch (this.tmpCol.dataType){
                        case "string" :
                            // VF added conditional
                            if (this.tmpCol.uniqueText){
        ...
        parse:{
            string: function(tmpCol,text){
                ...
                // VF added && tmpCol.uniqueText
                if (tmpCol.uniqueCount <= tmpCol.root.uniqueLength && tmpCol.uniqueText && tmpCol.uniqueText.indexOf(text) < 0){
        ...
        buildColumns: function() {
            ...
            jQuery.each(root.cols,function(i,v){
                ...
                if (v ){
					v.buildSort(sRow[i]);
                    // VF added && v.uniqueText
					if ((v.dataType == "date" && v.uniqueMonths.length > 1) || (v.dataType != "date" && v.uniqueText && v.uniqueText.length > 1)){
...
jQuery.extend(jQuery.colFilter,{
    defaults: {
        dataType: "number", //text/date/number
